...
int main()
{
	int p[10],at[10],bt[10],ct[10],tat[10],wt[10],bt2[10],pr[20],n,i,min,flag,count,time=0,condition;
	...
	for(i=0;i<n;i++)
		p[i]=i+1;
	printf("enter arrival times & burst times & priority\n\n");
	for(i=0;i<n;i++)
	{
		printf("enter at, bt, priority of P%d\n",i+1);
		scanf("%d",&at[i]);
		scanf("%d",&bt[i]);
		scanf("%d",&pr[i]);
		bt2[i]=bt[i];
